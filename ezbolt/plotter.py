...
def preview(boltgroup):
    ...
    axs[0].annotate(r"$x_{{cg}} = {:.1f} \quad {}$".format(boltgroup.x_cg, len_unit),
                    (xo,yo-dy*2), xycoords='axes fraction', fontsize=14, va="top", ha="left")
    axs[0].annotate(r"$y_{{cg}} = {:.1f} \quad {}$".format(boltgroup.y_cg, len_unit),
                    (xo,yo-dy*3), xycoords='axes fraction', fontsize=14, va="top", ha="left")
    axs[0].annotate(r"$I_{{x}} = {:.1f} \quad {}$".format(boltgroup.Ix, len_unit_pow3),
                    (xo,yo-dy*4), xycoords='axes fraction', fontsize=14, va="top", ha="left")
    axs[0].annotate(r"$I_{{y}} = {:.1f} \quad {}$".format(boltgroup.Iy, len_unit_pow3),
                    (xo,yo-dy*5), xycoords='axes fraction', fontsize=14, va="top", ha="left")
    axs[0].annotate(r"$J = {:.1f} \quad {}$".format(boltgroup.Iz, len_unit_pow3),
...
def plot_elastic(boltgroup, annotate_force=True):
    ...
    axs[0].annotate(r"$V_x = {:.1f} \quad {}$".format(boltgroup.Vx, force_unit),
                    (xo,yo-dy*8), xycoords='axes fraction', fontsize=14, va="top", ha="left")
    axs[0].annotate(r"$V_y = {:.1f} \quad {}$".format(boltgroup.Vy, force_unit),
                    (xo,yo-dy*9), xycoords='axes fraction', fontsize=14, va="top", ha="left")
    axs[0].annotate(r"$M_z = {:.1f} \quad {}$".format(boltgroup.torsion, moment_unit),
    ...
    axs[0].annotate(r"Bolt Demand = {:.2f} {}".format(boltgroup.bolt_demand, force_unit),
                    (xo,yo-dy*12), xycoords='axes fraction', fontsize=14, va="top", ha="left")
    axs[0].annotate(r"Bolt Capacity = {:.2f} {}".format(boltgroup.bolt_capacity, force_unit),
    ...
    for bolt in boltgroup.bolts:
        axs[1].plot([bolt.x],[bolt.y], 
                 ...
                 linestyle="none")
        if annotate_force:
            axs[1].annotate("({:.1f} {}, {:.1f} {})".format(bolt.vx_total, force_unit[0], bolt.vy_total, force_unit[0]),
                         xy=(bolt.x, bolt.y), 
                         xycoords='data', 
                         xytext=(0, -16), 
                         textcoords='offset points', 
...
def plot_ECR(boltgroup, annotate_force=True):
    ...
    yo = 0.95
    dy = 0.045
    unit = force_unit if boltgroup.ecc!=0 else moment_unit
    axs[0].annotate("Bolt Group Properties", 
                    (xo-0.03,yo), fontweight="bold",xycoords='axes fraction', fontsize=12, va="top", ha="left")
    axs[0].annotate(r"$N_{{bolts}} = {:.0f}$".format(boltgroup.N_bolt), 
                    (xo,yo-dy*1), xycoords='axes fraction', fontsize=14, va="top", ha="left")
    ...
    for bolt in boltgroup.bolts:
        axs[1].plot([bolt.x],[bolt.y], 
                 ...
                 linestyle="none")
        if annotate_force:
            axs[1].annotate("({:.1f} {}, {:.1f} {})".format(bolt.vx_ECR, force_unit[0], bolt.vy_ECR, force_unit[0]),
                         xy=(bolt.x, bolt.y), 
                         xycoords='data', 
                         xytext=(0, -16), 
                         textcoords='offset points', 
...
def plot_ICR(boltgroup, annotate_force=True):
    ...
    axs[0].annotate("Applied Loading", 
                    (xo-0.03,yo-dy*7), fontweight="bold",xycoords='axes fraction', fontsize=12, va="top", ha="left")
    axs[0].annotate(r"$P = {:.1f} \quad {}$".format(boltgroup.V_resultant, force_unit),
                    (xo,yo-dy*8), xycoords='axes fraction', fontsize=14, va="top", ha="left")
    axs[0].annotate(r"$\theta = {:.1f} \quad degs$".format(boltgroup.theta), 
                    (xo,yo-dy*9), xycoords='axes fraction', fontsize=14, va="top", ha="left")
    axs[0].annotate(r"$e_x = {:.2f} \quad {}$".format(boltgroup.ecc_x, len_unit),
                    (xo,yo-dy*10), xycoords='axes fraction', fontsize=14, va="top", ha="left")
    axs[0].annotate(r"$e_y = {:.2f} \quad {}$".format(boltgroup.ecc_y, len_unit),
    ...
    for bolt in boltgroup.bolts:
        axs[1].plot([bolt.x],[bolt.y], 
                 ...
                 linestyle="none")
        if annotate_force:
            axs[1].annotate("({:.1f} {}, {:.1f} {})".format(bolt.vx_ICR[-1], force_unit[0], bolt.vy_ICR[-1], force_unit[0]),
                         xy=(bolt.x, bolt.y), 
                         xycoords='data', 
                         xytext=(0, -16), 
                         textcoords='offset points', 
